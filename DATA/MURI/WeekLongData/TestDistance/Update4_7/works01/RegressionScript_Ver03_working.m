...
xtemp = exp(xtemp.*kron(ones(size(xtemp,1),1),[1000,500,5000,1]));
...
%split data
P = cvpartition(ytemp,'Holdout',0.5);
Ymat_train = ytemp(P.training);
Ymat_test = ytemp(P.test);
Xmat_train = xtemp(P.training,:);
Xmat_test = xtemp(P.test,:);

%Ymat_train = ytemp(logical(training_i));
%Ymat_test = ytemp(logical(testing_i));
%Xmat_train = xtemp(logical(training_i));
%Xmat_test = xtemp(logical(testing_i));
%% Autoscale
scaleData = [];
...
%%
save('P','P');
save('Ymat_train','Ymat_train');
save('Ymat_test','Ymat_test');
save('Xmat_test','Xmat_test');
save('Xmat_train','Xmat_train');
save('b','b');
save('scaleData','scaleData');