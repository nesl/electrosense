...
Ts = 1e-3;
t = (0:Ts:Ts*250-Ts)';
prbs_train = [t, prbs(1:250)];
prbs_test = [t, prbs(251:500)];
stem(prbs(1:250));
%% Build Model
...
rtwbuild(gcs);
tg.start; pause(2);
rx_train_front = tg.OutputLog(1:sampled_length,1:3); % there is an unknown zero value for the first element
rx_train_back = tg.OutputLog(1:sampled_length,4:6);
tx = tg.OutputLog(1:sampled_length,7);

% take out any DC bias
rx_average_train_front = mean(rx_train_front,1)';
for i=1:size(rx_train_front,2);
    rx_train_front(:,i) = rx_train_front(:,i) - ones(sampled_length,1).*rx_average_train_front(i);
end
rx_average_train_back = mean(rx_train_back,1)';
for i=1:size(rx_train_back,2);
    rx_train_back(:,i) = rx_train_back(:,i) - ones(sampled_length,1).*rx_average_train_back(i);
end

data_front = iddata(rx_train_front, tx, Ts_out); 
data_back = iddata(rx_train_back,tx,Ts_out);
% get state-space model using subspace method
[sys_train_front, x0_train_front] = n4sid(data_front,3); % 'best' extimates the order
[sys_train_back, x0_train_back] = n4sid(data_back,3); % 'best' extimates the order
%% Test model (manual)
