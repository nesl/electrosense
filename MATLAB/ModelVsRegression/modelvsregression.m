...
% for i=3:s1
%    %temp_b4tharray(i-1,:) = (b4tharray(i-2,:)+ b4tharray(i-1,:) + b4tharray(i,:))/3;
%    temp_bquadarray(i-1,:) = (bquadarray(i-2,:)+ bquadarray(i-1,:) + bquadarray(i,:))/3;
%    temp_AnormArray(i-1,:) = (AnormArray(i-2,:) + AnormArray(i-1,:) + AnormArray(i,:))/3;
% end
% %b4tharrayTrain = temp_b4tharray;
% bquadarrayTrain = temp_bquadarray;
% AnormArrayTrain = temp_AnormArray;
...
% REGRESSION 2 START
% Use predictions the best predictions 5,1,6,3 to look at worst predictions
% 4,2,7 
% stats2array = zeros(2,2);
% PolyArrayTrain = [ones(size(bAdj,1),1), bAdj(:,[5,1,6,3])];
% index = 0;
% for i = [2,4,7]
%     index = index + 1;
%     [b, bint, r, rint, stats] = regress(bquadarrayTrain(:,i),PolyArrayTrain);
%     stats2array(index,:) = [i,stats(1)];
% end
