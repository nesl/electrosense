...
for i = 1:nargin
    thisArg = varargin{i};
    if ~ischar(thisArg)
        error(message('python:InputsMustBeStrings'));
    end
    if i==1
        if exist(thisArg, 'file')==2
            ...
            error(message('python:FileNotFound', thisArg));
        end
    end
...
if isempty(cmdString)
    error(message('python:NopythonCommand'));
elseif ispc
    % PC
    ...
    if (status == 0)
        ...
        error(message('python:NoExecutable'));
    end
end

% Check for errors in shell command
if nargout < 2 && status~=0
    error(message('python:ExecutionError', result, cmdString));
end
