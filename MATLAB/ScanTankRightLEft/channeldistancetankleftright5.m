...
temp = [];
for j=1:10
    temp = [temp,tg.OutputLog(2:980,1:6);
    pause(pause_sec);
end
OutputLog{2} = tg.OutputLog(1:numberOfSamples,1:6); 
...
for i=2:length(trajectory)
    ...
    tg.start; pause(pause_sec);
    OutputLog{i+1} = tg.OutputLog(1:numberOfSamples,1:6); 
end
...
trans_signal = tg.OutputLog(1:1000,7);
